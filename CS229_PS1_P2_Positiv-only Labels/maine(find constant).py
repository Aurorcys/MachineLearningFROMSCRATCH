...
class LogisticRegressionNR:
    ...
    def predict(self, X):
        X_bias = np.c_[np.ones(X.shape[0]), X]
        probs = self.sigmoid(X_bias @ self.param)
        return (probs >= 0.5).astype(int)



df = pd.read_csv('ds3_valid.csv')
X = df[['x_1', 'x_2']].values
y = df['y'].values
t = df['t'].values
...
V_plus = X[t == 1]  # Only examples where TRUE label is 1
probs = model.sigmoid(np.c_[np.ones(V_plus.shape[0]), V_plus] @ model.param)
...
#predict
test = pd.read_csv('ds3_test.csv')
X_test = test[['x_1', 'x_2']].values
y_test = test['y'].values
y_t = test['t'].values
p_noisy = model.sigmoid(np.c_[np.ones(X_test.shape[0]), X_test] @ model.param)
p_corrected = p_noisy / alpha
p_corrected = np.clip(p_corrected, 0, 1)  # Ensure probabilities are valid

y_pred_corrected = (p_corrected >= 0.5).astype(int)

# Evaluate the model
accuracy = np.mean(y_pred_corrected == y_t)
print(f"Corrected Test Accuracy: {accuracy:.4f}")