...
def get_words(message):
    ...
    # *** START CODE HERE ***
    return ''.join(c if c.isalnum() or c.isspace() else ' ' for c in message.lower()).split()
    # *** END CODE HERE ***
...
def fit_naive_bayes_model(matrix, labels):
    ...
    priors = {}
    for c in classes:
        priors[c] = np.sum(labels == c) / n_samples
    #P(classes)
    ...
    for c in classes:
        X_c = matrix[labels == c]
        word_counts = X_c.sum(axis=0) + alpha              # Add alpha to every word
        total_words = word_counts.sum()                    # Sum includes alpha × vocab_size
        likelihoods[c] = word_counts / total_words
    
    return {'priors': priors, 'likelihoods': likelihoods, 'classes': classes}
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    for x in matrix:
        ...
        for c in classes:
            ...
            scores[c] += np.sum(x * np.log(likelihoods[c]))
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    """
    # *** START CODE HERE ***
    spam_likelihood = model['likelihoods'][1]
    ham_likelihood = model['likelihoods'][0]

    ratio = spam_likelihood / ham_likelihood
    ratio = np.log(ratio)
    top_indices = np.argsort(ratio)[::-1][:5]
...
def compute_best_svm_radius(train_matrix, train_labels, val_matrix, val_labels, radius_to_consider):
    ...
    for radii in radius_to_consider:
        predictions = svm.train_and_predict_svm(train_matrix, train_labels, val_matrix, val_labels, radii)
