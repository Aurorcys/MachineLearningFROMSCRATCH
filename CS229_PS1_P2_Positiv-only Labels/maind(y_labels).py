...
plt.scatter(X_test[:, 0], X_test[:, 1], c=y_test, cmap='bwr', edgecolors='k', label='True')
# boundary
xlim = plt.xlim()
ylim = plt.ylim()
xx, yy = np.meshgrid(
    np.linspace(xlim[0] - 1, xlim[1] + 1, 100),
    np.linspace(ylim[0] - 1, ylim[1] + 1, 100)
)
Z = model.sigmoid(np.c_[np.ones(xx.ravel().shape), xx.ravel(), yy.ravel()] @ model.param)
Z = Z.reshape(xx.shape)
plt.contour(xx, yy, Z, levels=[0.5], colors='red', linewidths=1)
